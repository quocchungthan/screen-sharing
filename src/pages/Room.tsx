...
const Room: React.FC = () => {
	...
	const { username, isMuted, isSharing, setSharing } = useUserStore();
	...
	useEffect(() => {
		...
		const initialize = async () => {
			try {
				...
				// ✅ Join the room (this emits to peers + updates store)
	...
	const handleToggleScreenShare = async () => {
		try {
			...
			toggleSharing(); // sync with other clients
		} catch (err) {
			console.error("Error toggling screen share:", err);
			setError(
				"Could not start screen sharing. Please check permissions and try again."
			);
		}
	};
			...
			>
				<div className="max-w-4xl mx-auto flex items-center justify-between">
					<div className="flex items-center space-x-2">
						...
						>
							{isSharing ? <StopCircle size={20} /> : <Share2 size={20} />}
						</button>
					</div>
